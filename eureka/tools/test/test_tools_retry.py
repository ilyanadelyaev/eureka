...
class TestRetry:
    def test__no_retry(self):
        @eureka.tools.retry.retry(Exception, 2)
        def f(v):
            v[0] -= 1
        #
        value = [2]
    ...
    def test__exception_not_match(self):
        @eureka.tools.retry.retry(AttributeError, 2)
        def f(v):
            v[0] -= 1
            raise RuntimeError
        #
    ...
    def test__0_attempts(self):
        @eureka.tools.retry.retry(Exception, 0)
        def f(v):
            v[0] -= 1
            raise Exception
        #
    ...
    def test__1_attempt(self):
        @eureka.tools.retry.retry(Exception, 1)
        def f(v):
            v[0] -= 1
            raise Exception
        #
    ...
    def test__default_attempts(self):
        @eureka.tools.retry.retry(Exception)
        def f(v):
            v[0] -= 1
            raise Exception
        #
